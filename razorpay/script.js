...
function initializeProductData() {
    ...
    // No GST for add_funds action - user should get exactly what they pay for
    const taxes = (productData.action === 'add_funds') ? 0 : (subtotal * 0.18); // 18% GST for plans only
...
async function handlePurchaseCompletion(method) {
    // For add_funds, use the base amount (no GST). For purchases, use total (with GST)
    const amountToRecord = (productData.action === 'add_funds') ? productData.subtotal : productData.total;
